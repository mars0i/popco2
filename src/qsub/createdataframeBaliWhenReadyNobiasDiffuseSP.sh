...
if [ -f bali.rdata ]; then
	cd ~/data
	qsub -l h_rt=18:00:00,vf=30G -N finddisg ~/p2/src/qsub/submitanything.job Rscript --no-init-file --verbose ~/p2/src/R/R/finddisagreementBaliNobiasDiffuseSP.R
	qsub -l h_rt=18:00:00,vf=40G -N createdf ~/p2/src/qsub/submitanything.job Rscript --no-init-file --verbose ~/p2/src/R/R/createdataframeBaliNobiasDiffuseSP.R
else
