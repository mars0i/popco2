...
(defn make-person
  ...
  (let [num-poss-propn-nodes (count (:node-vec propn-net))
    ...
    ;; regular nodes:
    (doseq [propn-id propn-ids]
      (cl/add-to-propn-net! pers propn-id))                     ; unmask propn nodes
    ...
    ;; regular nodes:
    (doseq [propn-id propn-ids]
      (cl/try-add-to-analogy-net! pers propn-id))              ; unmask analogy nodes (better to fill propn mask first)
