...
(defn popco-activn-attr
  [activn tick]
  [:attvalue {:for "popco-activn"
              :value (str activn)
              :start (str tick)
              :endopen (str (inc tick))}])


(defn new-node
  "Returns hashmap nodes with new entry for node popco-id."
  [nodes popco-id activn tick]
  (swap! popco-to-gexf-node-id 
         ug/assoc-if-new-throw-if-old popco-id (swap! node-id-num inc)) ; we should never encounter the same id twice.
  (assoc nodes 
         popco-id [:node {:id (str @node-id-num) :label popco-id :start (str tick)}
                   [:attvalues {} (popco-activn-attr activn tick)]
...
(defn update-node
  ...
  (let [[:node attrs
         [:attvalues {} & attrspecs]
         & otherspecs] (get nodes popco-id)]
    (assoc nodes 
           popco-id (into [:node attrs 
                           (conj
                             (into
                               [:attvalues {}]
                               (vec attrspecs))
                             (popco-activn-attr activn tick))]
                          otherspecs))))

;; Since we have to insert attrs into an old node spec, I'm storing node specs in a hash,
;; and then at the end can pass to toplevel graph function.
(defn node
  [nodes popco-id activn tick]
  (if (get nodes popco-id)
    (update-node nodes popco-id activn tick)
    (new-node    nodes popco-id activn tick)))
...
(defn new-edge
  ...
  [edges node1-id node2-id popco-wt tick]
...
(def gephi-dummy-nodes [(node {} ; FIXME
                              "dummy1" -1.0 0)
                        (node {} ; FIXME
                              "dummy2" 0.0 0)
                        (node {} ; FIXME
                          "dummy3" 1.0 0)]) ; make sure that Gephi ranking has limits -1, 1 for activns

(def gephi-dummy-edges [(edge {} ; FIXME
                              "dummy1" "dummy2" 1.0 0)
                        (edge {} ; FIXME
                          "dummy2" "dummy3" -1.0 0)])  ; make sure that Gephi ranking has limits -1, 1 for link weights
...
(defn nn-to-gephi-graph
  ...
  (gexf-graph (concat (nn-to-nodes nnstru) gephi-dummy-nodes)
              ...
              :dynamic
              0)) ; TODO allow modification of this
