...
load("dispersal/dispersal1/foci.rdata")
cb.ints <- foci2intervals(cb.foci)
cv.ints <- foci2intervals(cv.foci)
df <- dispersal1.df[dispersal1.df$bias!="none",]
names(df)[1] <- "isolate"
names(df)[2] <- "capture"
pdf()
...
bwplot(isolate + capture ~ bias, data=df, ylim=c(-1,1), coef=0, outer=T, panel=function(x, y, ...){panel.hanoi(x, y, col="gray90", border="gray90", breaks=if(panel.number()==1){cv.ints}else{cb.ints}, ...); panel.bwplot(x, y, ...); panel.points(tapply(y, factor(x), FUN = mean), pch=20, cex=1, col="red")}, par.settings=list(box.rectangle=list(col="blue"), box.umbrella=list(col="blue")), main=list(label="Summary of mean activations for two belief sets in 50 runs, time step 5000", fontface="bold"), xlab="biases\n\n(box: quartiles,  red: mean,  black: median,  gray: distribution)", ylab="per-run mean activations")
dev.off()

# and here's that bwplot line formatted more nicely:

bwplot(isolate + capture ~ bias, data=df, 
	ylim=c(-1,1), 
	coef=0, 
	outer=T, 
	panel=function(x, y, ...){
	...
	main=list(label="Summary of mean activations for two belief sets in 50 runs, time step 5000", fontface="bold"), # you can also use fontfamily, fontsize here
